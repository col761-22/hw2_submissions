...
int main(int argc, char** argv)
{
    ...
    string gspanFilename = argv[2];
    auto beginTime = high_resolution_clock::now();
    int tt =getGraphsFromStandardInput(filename, graphs, labelMap, graphIndexToID, beginTime);
    std::unordered_map<string, std::unordered_set<int>> binaryFeatureIndex;
    
    int graphCT = 0, maxSize = 15;    
    graphType::edge_iterator edgeItr, end;
    
    vector<vector<graphType>> featureGraphs(maxSize);
    vector<vector<std::unordered_set<int>>> featureGraphsTids(maxSize);

    for(auto &gph : graphs)
    {
        for(tie(edgeItr,end) = edges(gph); edgeItr != end; edgeItr++)
        {
            string label = std::to_string(get(edge_name, gph)[*edgeItr]);
            string beginNode = std::to_string(get(vertex_name, gph)[source(*edgeItr, gph)]);
            string endNode = std::to_string(get(vertex_name, gph)[target(*edgeItr, gph)]);
            string indexKey = "";
            string node1 = beginNode;
            string node2 = endNode;
            string edgeLabel = label;
            ...
            if(binaryFeatureIndex.find(indexKey) == binaryFeatureIndex.end())
            {
                binaryFeatureIndex[indexKey] = std::unordered_set<int>({graphCT});
                graphType gph;
                add_vertex(propNode(stoi(beginNode)), gph);
                add_vertex(propNode(stoi(endNode)), gph);
                add_edge(0,1, propEdge(stoi(label)), gph);
                featureGraphs[1].push_back(gph);
            }
            else
            {
                binaryFeatureIndex[indexKey].insert(graphCT);
            }
        }
        graphCT = graphCT + 1;
    }
    ...
    int i =0;
    while(i < featureGraphs[1].size())
    {
        graphType gph = featureGraphs[1][i];
        for(tie(edgeItr,end) = edges(gph); edgeItr != end; edgeItr++)
        {
            string label = "";
            string beginNode = "";
            ...
            indexKey = indexKey + node1 + "#" + node2 + "#" + edgeLabel;
            featureGraphsTids[1].push_back(binaryFeatureIndex[indexKey]);
        }
        i = i + 1;
    }
    
    vector<vector<graphType>> gspanFeatureGraphs(maxSize);
    vector<vector<std::unordered_set<int>>> gspanFeatureGraphsTids(maxSize);
    getGraphsSizeWiseFromGspan(gspanFilename, gspanFeatureGraphs, gspanFeatureGraphsTids);

    cout << "Time Taken for Reading Datasets : " << getTimeSpentInSec(beginTime) << endl;

    int kept = 0, actual = 0;
    float  gammaCut = 1.05000000;
    if (graphs.size() < 25000)
    {
        gammaCut = 1.20000000;
    }
    else if (graphs.size() <= 50000)
    {
        gammaCut = 1.10000000;
    }
    ...
    {
        int lkept = 0;
        actual = actual + gspanFeatureGraphs[ct].size();
        int j = 0;
        while(j < gspanFeatureGraphs[ct].size())
        {
            graphType gph = gspanFeatureGraphs[ct][j];
            if (kept <= 500)
            {
                float gamma = getGamma(gph, gspanFeatureGraphs, gspanFeatureGraphsTids[ct][j], featureGraphs, featureGraphsTids);
                if (gamma < gammaCut)
                {
                    string gam = "do nothing";
                }
                else
                {
                    featureGraphsTids[ct].push_back(gspanFeatureGraphsTids[ct][j]);
                    lkept = lkept +1;
                    featureGraphs[ct].push_back(gph);
                    kept = kept +1;
                }
            }
            j=j+1;
    ...
    cout << "Feature Index Size : " << kept ;
    cout << " original : " << actual << endl;
    cout << "Indexing Done... Now perform Querying";
    
    writebinaryFeatureIndex(binaryFeatureIndex);
    writefeatureGraphs(featureGraphs);
    writefeatureGraphsTids(featureGraphsTids);
    writegraphIndexToID(graphIndexToID);
    writegraphs(graphs);
